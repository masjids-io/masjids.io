...
// This is the handler for DELETE requests to /api/users/[id]
// It deletes a user by the ID provided in the URL.
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
    try {
      ...
      // 4. Get the user ID to delete from the dynamic route parameters.
      const { id } = params;
      if (!id) {
      ...
      if (!apiResponse.ok) {
        ...
        return NextResponse.json(
          { error: 'Failed to delete user.', details: errorResult },
          { status: apiResponse.status }
        );
      }
  
      // 8. If successful, return a success response with a 204 status (No Content).
